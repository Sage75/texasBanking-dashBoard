...
// ✅ Start session for all users (OTP + AJAX)
...
add_filter('wp_mail_from', function($email) {
    return 'verification@texascreditbenchmark.com';
});
add_filter('wp_mail_from_name', function($name) {
    return 'Texas Credit Benchmark';
});
...
/* --------------------------------
   🔐 VERIFY OTP
---------------------------------- */
add_action('wp_ajax_verify_otp', 'cbm_verify_otp');
add_action('wp_ajax_nopriv_verify_otp', 'cbm_verify_otp');

function cbm_verify_otp() {
    global $wpdb;
    $email = sanitize_email($_POST['user_email']);
    ...
    $row = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM $table WHERE email = %s AND otp = %s AND verified = 0 AND expires_at >= %s ORDER BY id DESC LIMIT 1",
        $email, $input_otp, current_time('mysql', 1)
    ));
...
function get_user_profile() {
    ...
    $email = sanitize_email($_SESSION['otp_email']);
    $table = $wpdb->prefix . 'profiles';

    $profile = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE email = %s", $email), ARRAY_A);

    if (!$profile) {
        wp_send_json_error(['message' => 'No profile found.']);
    } else {
...
function get_card_data() {
    ...
    $email = sanitize_email($_SESSION['otp_email']);
    $table = $wpdb->prefix . 'cards';

    $card = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE Email = %s", $email), ARRAY_A);

    if (!$card) {
        wp_send_json_error(['message' => 'Card not found']);
    } else {
...
add_action('wp_ajax_toggle_card_status', 'toggle_card_status');
add_action('wp_ajax_nopriv_toggle_card_status', 'toggle_card_status');


function toggle_card_status() {
    global $wpdb;

    if (!isset($_SESSION['otp_email'], $_POST['status'])) {
    ...
    if ($result !== false) {
        wp_send_json_success(['message' => 'Card status updated']);
    } else {
...
/* --------------------------------
   💸 ADD RECIPIENT & DEDUCT BALANCE
---------------------------------- */
add_action('wp_ajax_add_recipient', 'add_recipient');
add_action('wp_ajax_nopriv_add_recipient', 'add_recipient');
...
function add_recipient() {
    ...
    $user_email = sanitize_email($_SESSION['otp_email']);
    $recipient_table = $wpdb->prefix . 'recipients';
    $balance_table   = $wpdb->prefix . 'user_balance';

    $transfer_amount = floatval($_POST['transfer_amount'] ?? 0);
    $payment_method = sanitize_text_field($_POST['payment_method'] ?? '');

    $allowed = ['Debit Card', 'Credit Card', 'Bank Transfer'];
    ...
    $current_balance = $wpdb->get_var($wpdb->prepare(
        "SELECT balance FROM $balance_table WHERE email = %s",
        $user_email
    ));

    if ($current_balance === null) {
        wp_send_json_error(['message' => 'Balance not found']);
        return;
    }

    if ($transfer_amount > floatval($current_balance)) {
    ...
    $data = [
        ...
        'bank_address'    => sanitize_text_field($_POST['bank_address']),
    ...
    if (!$wpdb->insert($recipient_table, $data)) {
        wp_send_json_error(['message' => 'Insert failed', 'sql_error' => $wpdb->last_error]);
        return;
    }

    $wpdb->query($wpdb->prepare(
        "UPDATE $balance_table SET balance = balance - %f WHERE email = %s",
        $transfer_amount, $user_email
    ));
...
/* -------------------------------
   📊  GET LIVE BALANCE
--------------------------------*/
add_action('wp_ajax_get_user_balance', 'get_user_balance');
add_action('wp_ajax_nopriv_get_user_balance', 'get_user_balance');
...
function get_user_balance() {
    ...
    if (!isset($_SESSION['otp_email'])) {
        wp_send_json_error(['message' => 'Session expired or email not set']);
        return;
    }
    ...
    wp_send_json_success(['balance' => round(floatval($balance), 2)]);
}

/* -------------------------------
   📋 GET RECIPIENT LIST
--------------------------------*/
add_action('wp_ajax_get_recipients', 'get_recipients');
add_action('wp_ajax_nopriv_get_recipients', 'get_recipients');
...
function get_recipients() {
    ...
    $email = sanitize_email($_SESSION['otp_email']);
    $table = $wpdb->prefix . 'recipients';

    $results = $wpdb->get_results($wpdb->prepare(
        "SELECT id, full_name, account_number FROM $table WHERE user_email = %s",
        $email
    ));
...
/* -------------------------------
   📃 BANK STATEMENT DATA
--------------------------------*/
add_action('wp_ajax_get_all_recipient_data', 'get_all_recipient_data');
add_action('wp_ajax_nopriv_get_all_recipient_data', 'get_all_recipient_data');


function get_all_recipient_data() {
    global $wpdb;
    ...
    $results = $wpdb->get_results(
        $wpdb->prepare("SELECT * FROM $table WHERE user_email = %s ORDER BY created_at DESC", $email),
        ARRAY_A
    );
